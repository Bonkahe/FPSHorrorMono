...
{
    [Signal] public delegate void OnShotCameraImpulseEventHandler(Vector3 impulseDirection, float shotCameraImpulse);


    [ExportCategory("Shot Functionality")]
    [Export] public Array<AudioStream> GunshotSounds { get; set; } = new Array<AudioStream>();
    [Export] public Node3D PlayerBodyNode { get; set; }
    ...
    [Export] public float CameraShakePower { get; set; } = 0.3f;
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        if (IsAiming)
        {
            ...
            AimingIKContainer.GlobalPosition += HandOffsetPosition;
        ...
        else
        {
            ...
            RightHandIdleIKContainer.GlobalPosition += HandOffsetPosition;
            ...
            LeftHandIdleIKContainer.GlobalPosition += HandOffsetPosition;
    ...
    {
        if (currentFlashlightTarget != null && (!IsInstanceValid(currentFlashlightTarget) || !IsInFlashlightFieldOfView(currentFlashlightTarget.GlobalPosition)))
        {
            currentFlashlightTarget = null;
    ...
    {
        EmitSignal(SignalName.OnShotCameraImpulse, BarrelRayCast.GlobalTransform.Basis.Z, CameraShakePower);

        if (GunshotSounds.Count > 0)
        {
            GetTree().CallGroup("AudioQues", "PlayAudioQue3D", GunshotSounds[new RandomNumberGenerator().RandiRange(0, GunshotSounds.Count - 1)], BarrelEnd.GlobalPosition, 5);
        }
        ...
        if (BarrelRayCast.IsColliding())
        {
            ...
            if (ImpactedTarget.GetChildren().FirstOrDefault(x => x.Name == "Damageable") is DamageableObject damageable)
            {
                ...
                damageable.HitObject(hitLocation, -BarrelRayCast.GlobalTransform.Basis.Z * ImpactForce, PlayerBodyNode);
            }
