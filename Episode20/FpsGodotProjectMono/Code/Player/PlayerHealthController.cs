...
public partial class PlayerHealthController : Node
{
    ...
    [Export] public Array<AudioStream> HurtSounds { get; set; } = new Array<AudioStream>();
    [Export] public AudioStream PlayerDeathSound { get; set; }
    ...
    public void OnDamaged(Vector3 hitLocation, Vector3 force, Node3D AggressorBodyNode)
    {
        ...
        {
            GetTree().CallGroup("AudioQues", "PlayAudioQue", HurtSounds[new RandomNumberGenerator().RandiRange(0, HurtSounds.Count - 1)], 0);
        }
        ...
        if (CurrentHealth == 0)
        {
            GetTree().CallGroup("AudioQues", "PlayAudioQue", PlayerDeathSound, 0);
