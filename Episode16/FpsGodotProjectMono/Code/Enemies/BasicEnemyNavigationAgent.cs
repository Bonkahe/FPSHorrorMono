...
public partial class BasicEnemyNavigationAgent : RigidBody3D
{
    ...
    public override void _Ready()
    {
        ...
        rng.Randomize();
        LaunchAttemptInterval = rng.RandfRange(0, MinimumLaunchDelay);
    }
    ...
    public void SetNewTarget(Vector3 newTarget)
    {
        ...
        {
            float estimatedDuration = LastWaypoint.DistanceTo(waypoint) / MaximumVelocity;

            if (LastOnCeiling && rng.Randf() < currentPathChance * 2.0f)
            {
                if (DebugActive)
                {
            ...
            if (!LastOnCeiling)
            {
                ...
                {
                    CurrentUpVector = Vector3.One + (Vector3.One * (rng.Randf() - 0.5f) * 2.0f);
                }
                else
                {
