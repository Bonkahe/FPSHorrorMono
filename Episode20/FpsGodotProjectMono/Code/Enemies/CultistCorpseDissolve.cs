...
{
    [Export] public Array<AudioStream> DissolveSounds { get; set; } = new Array<AudioStream>();
	...
	public override void _Process(double delta)
	{
        ...
        else
        {
            if (!spawnedParticles)
            {
                if (DissolveSounds.Count > 0)
                {
                    GetTree().CallGroup("AudioQues", "PlayAudioQue3D", DissolveSounds[new RandomNumberGenerator().RandiRange(0, DissolveSounds.Count - 1)], DissolveSpawnPoints[0].GlobalPosition, 0);
                }
