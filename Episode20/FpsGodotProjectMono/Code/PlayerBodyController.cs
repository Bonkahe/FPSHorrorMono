...
public partial class PlayerBodyController : CharacterBody3D
{
	...
	{
        ///Used to allow rendering videos to be smoother.
        Engine.MaxFps = 60;
	...
	public override void _PhysicsProcess(double delta)
	{
        ...
        else
        {
            if (lastYVelocity < -CameraShake_FallingBias)
            {
                ...
                {
                    GetTree().CallGroup("AudioQues", "PlayAudioQue3D", FootstepSounds[new RandomNumberGenerator().RandiRange(0, FootstepSounds.Count - 1)], CameraNode.GlobalPosition + (Vector3.Down * 2.0f), 3);
                }
            }
        ...
        if (walkingSway_CurrentValue > 0)
        {
            ...
            if (stepBounce == WalkingSway_MaxSwayHandsHeight && FootstepSounds.Count > 0)
            {
                GetTree().CallGroup("AudioQues", "PlayAudioQue3D", FootstepSounds[new RandomNumberGenerator().RandiRange(0, FootstepSounds.Count - 1)], CameraNode.GlobalPosition + (Vector3.Down * 2.0f), -2);
		...
		Vector3 direction = CameraNode.Transform.Basis * new Vector3(inputDir.X, 0, inputDir.Y);
        direction.Y = 0;
